...
var predict_url = "https://656cde41b10a.ngrok-free.app/predecir"
...
func configurar_partida():
	...
	if not GameManager.config_proximo_nivel.is_empty():
		...
		if modo_juego == Modos.CONTRARRELOJ:
			...
			timer_nivel.one_shot = true # Forzamos que se detenga al llegar a 0
			
			if not timer_nivel.timeout.is_connected(finalizar_juego):
